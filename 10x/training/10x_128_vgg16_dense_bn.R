...
#if we continue to train a model from a checkpoint
# load_model_weights_hdf5(model, "D:\\dl\\10x\\models\\weights.15-0.9311.hdf5", by_name = TRUE)

#if we use a pretrained model
# model <- load_model_hdf5(filepath = "D:\\dl\\10x\\models\\best_model")
...
history <- model %>% fit_generator( #fit the model batch by batch to reduce the requirement of RAM
  ...
  callbacks=list(callback_csv_logger("D:\\dl\\10x\\log\\logger.csv",separator = ",", append=TRUE),
                 ...
                 callback_reduce_lr_on_plateau(monitor = "val_loss", factor = 0.1,verbose=1, mode="auto", 
                                               patience=6, min_delta=0.002, cooldown = 3),
  ...
  initial_epoch = 15 #the index of the first step will be (initial_epoch + 1)
)
