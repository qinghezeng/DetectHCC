...
Only for the case FCN without overlapping. That is to say yhe stride is 64p.
...
#%%
slide_name='R10_004_22- 2017-08-25 16.48.22.ndpi'
...
#blended.save(out_path+"blended_10x.png")
#
#plt.figure(figsize=(10,10))
#plt.imshow(blended)
#
...
#file=out_path+"features_10x.npy"
#np.save(file,features_map)
##load
##zz=np.load(file)
#file=out_path+"pb_10x.npy"
#np.save(file,pb_map)
...
# =============================================================================
pb_map=np.load(out_path+"_pb_stride64p_10x.npy")

#build a probability map
heatmap=np.transpose(pb_map[:,:,0]) #1 is tum
#heatmap_im = Image.fromarray(np.uint8(heatmap*255))
...
plt.title('ROC curve: VGG16 on '+slide_name[:-5].split('- ')[0] + ' stride64p 10x' )
plt.legend(loc="lower right")
plt.savefig(out_path + '_vgg16_roc_stride64p_10x.jpg')
plt.show()
plt. close()
...
#                    FNError.append([i, j])
#                else:
#                    crop.save('E:\\deeplearning\\Hepatocarcinomes\\SlidesClassification\\FalseSample\\TrainingSet\\FN_10x\\'+slide_name[:-5].split(' ')[0]+'_'+str(FN).zfill(5)+'.tif')
                FN = FN + 1
        else:
...
#                   FPError.append([i, j])
#                else:
#                    crop.save('E:\\deeplearning\\Hepatocarcinomes\\SlidesClassification\\FalseSample\\TrainingSet\\FP_10x\\'+slide_name[:-5].split(' ')[0]+'_'+str(FP).zfill(5)+'.tif')
                FP = FP + 1
            else :
                TN = TN + 1
...
if os.path.isfile(out_path.split('R')[0]+'vgg16_performance_training_stride64p_10x.csv'):
    performance.to_csv(out_path.split('R')[0]+'vgg16_performance_training_stride64p_10x.csv', index=False, header=False, mode='a')
else:
    performance.to_csv(out_path.split('R')[0]+'vgg16_performance_training_stride64p_10x.csv', index=False, header=True, mode='a')
...
plot_confusion_matrix(cm_normalized, title='Normalized confusion matrix on '+slide_name[:-5].split('- ')[0] + ' stride64p 10x')
# show confusion matrix
plt.savefig(out_path+'_confusion_matrix_stride64p_10x.png', format='png')
#plt.show()
plt.close()