...
slide_name='HMNT0264 - 2017-07-24 13.09.22.ndpi'
...
print("Best level for 10x :"+str(best_level))
...
# =============================================================================
pb_map=np.load(out_path+"_pb_10x.npy")
...
plt.title('ROC curve: VGG16 on '+slide_name[:-5].split(' ')[0]+' 10x')
plt.legend(loc="lower right")
plt.savefig(out_path + '_vgg16_roc_10x.jpg')
plt.show()
plt. close()
...
if os.path.isfile(out_path.split('HMNT')[0]+'vgg16_performance_training_10x.csv'):
    performance.to_csv(out_path.split('HMNT')[0]+'vgg16_performance_training_10x.csv', index=False, header=False, mode='a')
else:
    performance.to_csv(out_path.split('HMNT')[0]+'vgg16_performance_training_10x.csv', index=False, header=True, mode='a')
...
plot_confusion_matrix(cm_normalized, title='Normalized confusion matrix on '+slide_name[:-5].split('- ')[0]+' 10x')
# show confusion matrix
plt.savefig(out_path+'_confusion_matrix_10x.png', format='png')
#plt.show()
plt.close()