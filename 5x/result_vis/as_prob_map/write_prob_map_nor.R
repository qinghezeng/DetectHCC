...
# Save a probability map of Normal class as (orginal size + border) (5x)
    ...
    for(rr in hor_field)
    {
      ...
      for(r in every_label_size) #in fact is process by column
      {
        for(c in every_label_size) #1, 65, 129, ..., 449
        {
          r_ = r+label_size-1
          c_ = c+label_size-1
          label_field[c:c_,r:r_] = prob[j+1]
          j = j+2
        }
      }
...
writeTIFF(label, paste(path,"\\prob_map_nor",".tiff",sep=""),bits.per.sample=16L)
