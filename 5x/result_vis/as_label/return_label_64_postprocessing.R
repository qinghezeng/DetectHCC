#RScript for Visiopharm APP, to reimport the results after post-processing
...
slide_name="test\\HMNT0784"
######################################################

path1=paste("E:\\deeplearning\\data\\5x\\detector\\",slide_name,sep="")
path=paste(path1,"\\results\\label",sep="")
prob <- c(t(read.table(paste(path1,"\\results\\vgg_dense_bn_prob_256",sep=""), header = TRUE, sep = ",")))
...
for(c in every_64)
{
  ...
  for(r in every_64)
  {
    ...
    #   j = j+2
    # }
    label[r:r_63,c:c_63] = prob[j] #8 bits prob map to 256 labels in visiopharm 
    j = j+2
  }
}
