...
#Use the 64*64 annotated subtiles as data, this script uses 100k subtiles of each class for training (0.8) and validation (0.2).
...
base_model <- application_nasnetlarge(include_top = FALSE, 
                                      weights = "imagenet",
...
# model <- load_model_hdf5(filepath = "D:\\dl\\5x\\nasnet_large\\best_model")
...
history <- model %>% fit(
  ...
  callbacks=list(callback_csv_logger("D:\\dl\\5x\\log\\logger.csv",separator = ",", append=TRUE),
                 ...
                 callback_reduce_lr_on_plateau(monitor = "val_acc", factor = 0.1,verbose=1, mode="auto", 
                                               patience=6, min_delta=0.0015),
                 callback_early_stopping(monitor="val_acc", min_delta = 0.0005, patience=13, mode="auto"))
)
