...
base_model <- application_vgg16(include_top = FALSE, weights = "imagenet",
                           input_tensor = NULL, input_shape = c(64,64,3), pooling = NULL)
...
predictions <- base_model$output %>% 
  ...
  layer_dropout(0.5) %>% #here we add a dropout layer to avoid overfitting
  layer_dense(units = 4096, activation = 'relu') %>%
  layer_dropout(0.5) %>%
...
history <- model %>% fit(
  ...
  callbacks=list(callback_csv_logger("D:\\dl\\5x\\log\\logger.csv",separator = ",", append=TRUE),
                 ...
                 callback_early_stopping(monitor="val_acc", min_delta = 0.0005, patience=10, mode="auto"))
)
