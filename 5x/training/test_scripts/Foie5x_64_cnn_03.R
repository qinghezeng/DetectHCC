...
for (i in c(1:length(lpos))){
  pos[i,,,]=imageData(readImage(paste(path,"\\",lpos[i],sep="")))
...
opt <- optimizer_rmsprop(lr = 0.001)
...
if(!data_augmentation){
  ...
  history <- model %>% fit(
    ...
    callbacks=list(callback_csv_logger("D:\\qinghe\\log\\log.csv",separator = ",", append=TRUE),
                   ...
                   callback_model_checkpoint(filepath="D:\\qinghe\\models\\weights.{epoch:03d}-{val_acc:.4f}.hdf5",
...
model %>% load_model_weights_hdf5("D:\\qinghe\\models\\essai1\\weights.105-0.95.hdf5")
...
test_result <- list(length(l_test),test_loss$loss,test_loss$acc,length(ypos_test),
                    length(yneg_test),n_false,n_false_pos,n_false_neg,false_pos,false_neg)
