...
patch_size=64
#attention! if want to use 1.0, use the classification script for slide and the evalutaion script with 64p.
stride = 0.125 #in % of patch width: 0.125(8), 0.25(16), 0.5(32), 0.75(48)
...
heatmap_im.save(out_path + '_PbMap8bit' + stride_string + '.png')

#with colormap
heatmap_im = Image.fromarray(np.uint8(cm.plasma(heatmap)*255))
heatmap_im.save(out_path + '_PbMap8bit_plasma' + stride_string + '.png')
...
blended.save(out_path + '_blended' + stride_string + '.png')
...
#file= out_path + '_features' + stride_string + '.npy'
#np.save(file,features_map)
#load
#zz=np.load(file)
file= out_path + '_pb' + stride_string + '.npy'
np.save(file,pb_map)
