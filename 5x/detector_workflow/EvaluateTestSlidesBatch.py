...
#    plt.show()
            ...
            if annotations[i, j] == 255:
                if heatmap[i, j] > thresholds[best_threshold]:
                    ...
                    try:
                        ...
                        crop.save('E:\\deeplearning\\Hepatocarcinomes\\SlidesClassification\\FalseSample\\TestSet\\FN_5x\\'+slide_name[:-5].split(' ')[0]+'_'+str(FN).zfill(5)+'.tif')
                    FN = FN + 1
                ...
                if heatmap[i, j] > thresholds[best_threshold]:
                    try:
                       ...
                       FPError.append([i, j])
                        ...
                        crop.save('E:\\deeplearning\\Hepatocarcinomes\\SlidesClassification\\FalseSample\\TestSet\\FP_5x\\'+slide_name[:-5].split(' ')[0]+'_'+str(FP).zfill(5)+'.tif')
                    FP = FP + 1
                else :
                    TN = TN + 1
    ...
    if os.path.isfile(out_path.split('HMNT')[0]+'vgg16_performance_test.csv'):
        performance.to_csv(out_path.split('HMNT')[0]+'vgg16_performance_test.csv', index=False, header=False, mode='a')
    else:
        performance.to_csv(out_path.split('HMNT')[0]+'vgg16_performance_test.csv', index=False, header=True, mode='a')
