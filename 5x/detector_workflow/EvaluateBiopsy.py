...
slide_name='R10_004_21- 2017-08-25 16.40.35.ndpi'
...
# =============================================================================
pb_map=np.load(out_path+'_pb' + stride_string + '.npy')
...
#heatmap_im_ajusted.save(out_path+"_PbMap8bit_vis.tiff", dpi=(res, res))
...
plt.title('ROC curve: VGG16 on '+slide_name[:-5].split('- ')[0] + ' ' + stride_string[1:])
plt.legend(loc="lower right")
plt.savefig(out_path + '_vgg16_roc' + stride_string + '.jpg')
plt.show()
...
#                    FNError.append([i, j])
#                else:
#                    crop.save('E:\\deeplearning\\Hepatocarcinomes\\BiopsiesClassification\\FalseSample\\TestSet\\FN\\'+ stride_string + '\\' + slide_name[:-5].split('- ')[0]+'_'+str(FN).zfill(5)+'.tif')
                FN = FN + 1
        else:
...
#                   FPError.append([i, j])
#                else:
#                    crop.save('E:\\deeplearning\\Hepatocarcinomes\\BiopsiesClassification\\FalseSample\\TestSet\\FP\\' + stride_string + '\\' +slide_name[:-5].split('- ')[0]+'_'+str(FP).zfill(5)+'.tif')
                FP = FP + 1
            else :
                TN = TN + 1
...
if os.path.isfile(out_path.split('R')[0]+'vgg16_performance_training' + stride_string + '.csv'):
    performance.to_csv(out_path.split('R')[0]+'vgg16_performance_training' + stride_string + '.csv', index=False, header=False, mode='a')
else:
    performance.to_csv(out_path.split('R')[0]+'vgg16_performance_training' + stride_string + '.csv', index=False, header=True, mode='a')
...
plot_confusion_matrix(cm_normalized, title='Normalized confusion matrix on '+slide_name[:-5].split('- ')[0] + ' '+stride_string[1:])
# show confusion matrix
plt.savefig(out_path+'_confusion_matrix' + stride_string + '.png', format='png')
#plt.show()
plt.close()
