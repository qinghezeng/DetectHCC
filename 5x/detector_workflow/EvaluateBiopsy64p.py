...
# =============================================================================
pb_map=np.load(out_path+"_pb_stride64p.npy")
...
plt.title('ROC curve: VGG16 on '+slide_name[:-5].split('- ')[0] + ' stride64p' )
plt.legend(loc="lower right")
plt.savefig(out_path + '_vgg16_roc_stride64p.jpg')
plt.show()
plt. close()
...
#                    FNError.append([i, j])
#                else:
#                    crop.save('E:\\deeplearning\\Hepatocarcinomes\\SlidesClassification\\FalseSample\\TrainingSet\\FN\\'+slide_name[:-5].split(' ')[0]+'_'+str(FN).zfill(5)+'.tif')
                FN = FN + 1
        else:
...
#                   FPError.append([i, j])
#                else:
#                    crop.save('E:\\deeplearning\\Hepatocarcinomes\\SlidesClassification\\FalseSample\\TrainingSet\\FP\\'+slide_name[:-5].split(' ')[0]+'_'+str(FP).zfill(5)+'.tif')
                FP = FP + 1
            else :
                TN = TN + 1
...
if os.path.isfile(out_path.split('R')[0]+'vgg16_performance_training_stride64p.csv'):
    performance.to_csv(out_path.split('R')[0]+'vgg16_performance_training_stride64p.csv', index=False, header=False, mode='a')
else:
    performance.to_csv(out_path.split('R')[0]+'vgg16_performance_training_stride64p.csv', index=False, header=True, mode='a')
...
plot_confusion_matrix(cm_normalized, title='Normalized confusion matrix on '+slide_name[:-5].split('- ')[0] + ' stride64p')
# show confusion matrix
plt.savefig(out_path+'_confusion_matrix_stride64p.png', format='png')
#plt.show()
plt.close()