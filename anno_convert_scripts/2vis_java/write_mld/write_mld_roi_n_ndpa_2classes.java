//To write annotation named by its index exacted from a ndpa file into a mld file as ROIs. 
//Annotation name format: Tn / NTn, n is a natural number.
package write_mld;
...
public class write_mld_roi_n_ndpa_2classes {
	...
	public static void main(String argv[]) throws IOException {
		...
		String f = "W:\\HES HCC DL Projet\\HMNT0055_bis - 2017-07-13 22.57.41.ndpi.ndpa";		
		read_xml_n_2classes.set_file(f);
		//To get all the information about annotation objects from the ndpa file
		read_xml_n_2classes.read_data();
		nlabel = read_xml_n_2classes.get_anno_num();
		shape = read_xml_n_2classes.get_anno_shape();
		type = read_xml_n_2classes.get_anno_type();
		npoint = read_xml_n_2classes.get_point_num();
		for(int i=0; i<npoint.length; i++) {
			sumpoint = sumpoint + npoint[i];
		}
		point = read_xml_n_2classes.get_point();
		Rec= new double[] {0.0, 0.0, 3.4028234663852886E38, 3.4028234663852886E38, 0.0};
		
		//To specify a name for the mld file to create.
		File outFile = new File("D:\\vis_db\\ndpview_test\\roi_22.mld");
