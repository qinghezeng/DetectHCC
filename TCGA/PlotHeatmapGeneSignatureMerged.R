...
# Paths
in_path <- "/media/visiopharm5/WDGold/deeplearning/Hepatocarcinomes/TCGA/merge_series"
out_path <- "/media/visiopharm5/WDGold/deeplearning/Hepatocarcinomes/TCGA/merge_series"
...
gene_signature = "Gajewski_13G_Inflammatory" # Inflammatory, Gajewski_13G_Inflammatory, 6G_Interferon_Gamma, Interferon_Gamma_Biology, T-cell_Exhaustion, Ribas_10G_Interferon_Gamma
...
if (trans == "zscore") {
  if (datas == "full") {
    data <- as.matrix(as.data.frame(read.csv(paste(in_path, "final_log2-zscore-centering_merged_mondor_tcga.csv",
                                                   sep = "/"), sep="\t", row.names = 1)))
  }
...
if (sangro_filtered) {
  ...
  } else if (gene_signature == "6G_Interferon_Gamma")  {
...
q <- my_heatmap(bottom_annotation = b) + rowAnnotation(foo = anno_text(rownames(data), gp = gpar(fontsize = 18, col = row_label_colors)))
