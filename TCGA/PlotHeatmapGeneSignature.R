...
out_path <- "/media/visiopharm5/WDGold/deeplearning/Hepatocarcinomes/TCGA/validation_336_filter_before"
...
gene_signature = "Inflammatory" # Inflammatory, Gajewski_13G_Inflammatory, 6G_Interferon_Gamma, Interferon_Gamma_Biology, 
...
if (trans == "zscore") {
  if (datas == "full") {
    data <- as.matrix(as.data.frame(read.csv(paste(in_path, "fpkm_final_log2-zscore_336.csv",
                                                   sep = "/"), sep="\t", row.names = 1)))
  }
...
if (sangro_filtered) {
  ...
  # height = 0.6 * length(geneS) + 1.3150
  
  data <- data[geneS,]
  datas = gene_signature
}

# filter the 336 cases with slides of good quality, better to do it in Python before Z-Score
...
my_heatmap <- function(bottom_annotation = NULL) {
  pheatmap(data_scaled,
           ...
           top_annotation = HeatmapAnnotation(df = cluster_col, 
                                              ...
                                              # annotation_name_side = "left", annotation_name_offset="8cm"),
...
ht_height = unit(3, "mm")*nrow(data_scaled) #+ unit(4, "mm"),
...
q <- my_heatmap(bottom_annotation = b) + rowAnnotation(foo = anno_text(rownames(data), rot = 16,  # rotation
                                                                       gp = gpar(fontsize = 28, col = row_label_colors))) #18
...
write.table(res, file= paste("/media/visiopharm5/WDGold/deeplearning/Hepatocarcinomes/TCGA/clust_res_final",
                             "/tcga_sample_clusters_",datas,"_",trans,"_",alg,"_",linkage, "_", dst,"_",ncluster_col,sreorder,".csv", sep = ""),
            sep=",", quote=F, row.names=FALSE)
