...
# Dataset
datas = "filtered-4" # full, filtered-05, filtered-1, filtered-4, filtered-6, filtered-Sia
...
if (twoclusters_filtered) {
  ...
  gene_2clusters  <- as.data.frame(read.csv(paste0(out_path,"\\gene_clusters_sum_filter-4.csv"), sep=","))
...
# filter the 336 cases with slides of good quality
data = data[, !colnames(data) %in% c("A216.01A","A8YO.01A", "AACY.01A", "A5RF.01A")]
...
if (alg == "km") {
  ...
  # export
  write.table(cluster_row_named, file= paste(out_path, "\\gene_clusters_",datas,"_",trans,"_",alg, "_", dst,"_",ncluster_row,sreorder,".csv", sep = ""),
  ...
  # export
  write.table(rout, file= paste(out_path, "\\gene_clusters_",datas,"_",trans,"_",alg,"_",linkage, "_", dst,"_",ncluster_row,sreorder,".csv", sep = ""),
              sep=",", quote=F, row.names=FALSE)
}
...
if (alg == "km") {
  ...
  # export
  write.table(cluster_col_named, file= paste(out_path, "\\sample_clusters_",datas,"_",trans,"_",alg, "_", dst,"_",ncluster_col,sreorder,".csv", sep = ""),
              sep=",", quote=F, row.names=FALSE)
...
# Export heatmap
if (alg == "km") {
  pdf(paste(out_path, "\\clustermap_",datas,"_",trans,"_",alg,"_",dst,"_",ncluster_row,"_",ncluster_col,sreorder,".pdf", sep = ""),
...
# 
# if (alg == "km") {
#   pdf(paste(out_path, "\\removed_genes_std_descending_",datas,"_",trans,"_",alg,"_",dst,"_",ncluster_col,
#             sreorder,".pdf", sep = ""), compress=FALSE, width=width, height=height)
# } else if (alg == "hc") {
#   pdf(paste(out_path, "\\removed_genes_std_descending_",datas,"_",trans,"_",alg,"_",linkage,"_",dst,"_",
